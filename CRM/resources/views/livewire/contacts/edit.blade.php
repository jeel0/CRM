<x-slot name="header">
    <div class="flex justify-between w-full">
        ...
        <a href="{{ url('contacts' , $contact->id ) }}">
            <button wire:click="viewContact" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded inline-flex items-center gap-2">
                <i class="fa fa-user h-5 w-5"></i>
                <span>{{ ('Back To Contact') }}</span>
            </button>
        </a>
        <div class="bg-blue-500 hover:bg-white hover:border-blue-500 text-white hover:text-blue-500 hidden"></div>
    </div>
</x-slot>

<div class="max-w-7xl flex relative w-full justify-center gap-5 px-7 h-full min-h-[450px] mt-6 mx-auto">
    @if (session()->has('message') && $showFlash)
    <div id="contact-edit-flash-message" class="fixed z-40 top-0 flex items-center w-full px-6" role="alert">
        <div id="contact-edit-flash-message" class="flex items-center border-gray-300 border-2 p-4 w-full text-teal-900 bg-white rounded-lg shadow" role="alert">
            <div class="inline-flex flex-shrink-0 justify-center items-center w-8 h-8 text-green-500 rounded-lg">
                <i class="fa fa-check"></i>
                <span class="sr-only">Success</span>
            </div>
            <div class="ml-3 text-sm font-normal">{{ session('message') }}</div>
            <button wire:click="closeFlash" type="button" class="ml-auto -mx-1.5 -my-1.5 bg-white text-gray-400 hover:text-gray-900 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex h-8 w-8" aria-label="Close">
